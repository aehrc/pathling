...
public class LoincLoader {
    ...
    void execute(String loincUrl, String loincVersion, int expansionPageSize) throws SQLException {
        ...
        try {
            if (!dimensionExists(loincUrl, loincVersion)) {
                ...
                // TODO: Use the JDBC API for this.
                execute("COMMIT");
                double elapsedSecs = TimeUnit.SECONDS.convert(System.nanoTime() - start, TimeUnit.NANOSECONDS);
                logger.info("Built LOINC code dimension in " + String.format("%.1f", elapsedSecs) + " s");
                metadataWriter.write();
            } else {
                logger.info("LOINC Dimension already exists, skipping load: " + loincUrl + ", " + loincVersion);
            }
        } finally {
    ...
    private List<String> prepareValues(PreparedStatement insertStatement, LoincCode loincCode) throws SQLException {
    ...
    private String getPropertyValue(LoincCode loincCode, String property) {
        if (loincCode.getLookupResponse() == null) return null;
        Predicate<Parameters.ParametersParameterComponent> partMatchesProperty =
                part -> part.getName().equals("code") && ((CodeType) part.getValue()).asStringValue().equals(property);
        ...
        Predicate<Parameters.ParametersParameterComponent> valuePart = part -> part.getName().startsWith("value");
        try {
