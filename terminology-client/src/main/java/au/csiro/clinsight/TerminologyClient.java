...
public class TerminologyClient {
    ...
    public ValueSet expand(UriType url, IntegerType count, IntegerType offset) {
        ...
        logger.info("Expanded " + result.getExpansion().getContains().size() + " codes from " + url.asStringValue() +
                            " ValueSet in " + String.format("%.1f", elapsedMs) + " ms");
        return result;
    }

    /**
     * Performs a type-level CodeSystem$translate operation against the configured terminology server.
     *
    ...
    public Parameters translate(CodeType code, UriType system, UriType source, UriType target) {
        if (code == null || system == null || source == null || target == null) {
            throw new IllegalArgumentException("Must provide non-null code, system, source and target arguments.");
        }
        Parameters inParams = new Parameters();
    ...
    public Bundle searchStructureDefinitionsByUrl(String url) {
        return searchStructureDefinitionsByUrlCache.computeIfAbsent(url, u -> {
            ...
            double elapsedMs = TimeUnit.MILLISECONDS.convert(System.nanoTime() - start, TimeUnit.NANOSECONDS);
            logger.info(
                    "Searched for StructureDefinitions matching " + u + " in " + String.format("%.1f", elapsedMs) +
                            " ms");
            return result;
        });
    }
    ...
    public ElementDefinition getElementDefinitionForProperty(Resource resource, Property property) {
        String structureDefinitionUrl = "http://hl7.org/fhir/StructureDefinition/" + resource.fhirType();
        ...
        Optional<StructureDefinition> maybeStructureDefinition =
                results.getEntry().stream().map(entry -> (StructureDefinition) entry.getResource()).findFirst();
