...
import au.csiro.pathling.encoders2.SchemaTraversalEx.{isCollection, isSingular}
import ca.uhn.fhir.context.{BaseRuntimeElementDefinition, _}

import scala.collection.convert.ImplicitConversions._


trait SchemaVisitorEx[DT, SF] {
  ...
  def buildValue(childDefinition: BaseRuntimeChildDefinition, elementDefinition: BaseRuntimeElementDefinition[_], elementName: String,
                 compositeBuilder: (SchemaVisitorEx[DT, SF], BaseRuntimeElementCompositeDefinition[_]) => DT): Seq[SF]
  ...
  def enterChild(childDefinition: BaseRuntimeChildDefinition): SchemaVisitorEx[DT, SF] = this

  def enterChoiceOption(choiceDefinition: RuntimeChildChoiceDefinition, optionName: String): SchemaVisitorEx[DT, SF] = this
}


trait SchemaVisitorWithTypeMappings[DT, SF] extends SchemaVisitorEx[DT, SF] {
  ...
  def buildSimpleValue(childDefinition: BaseRuntimeChildDefinition, elementDefinition: BaseRuntimeElementDefinition[_], elementName: String,
                       compositeBuilder: (SchemaVisitorEx[DT, SF], BaseRuntimeElementCompositeDefinition[_]) => DT): DT = {
...
class SchemaTraversalEx[DT, SF](maxNestingLevel: Int) {
   ...
   */
  def enterResource(ctx: SchemaVisitorEx[DT, SF], resourceDefinition: RuntimeResourceDefinition): DT = {
    EncodingContext.runWithContext {
      enterComposite(ctx, resourceDefinition)
    }
  }

  def enterComposite(ctx: SchemaVisitorEx[DT, SF], definition: BaseRuntimeElementCompositeDefinition[_]): DT = {
    visitComposite(ctx, definition)
  }

  def visitComposite(ctx: SchemaVisitorEx[DT, SF], definition: BaseRuntimeElementCompositeDefinition[_]): DT = {
  ...
  def visitChild(ctx: SchemaVisitorEx[DT, SF], childDefinition: BaseRuntimeChildDefinition): Seq[SF] = {
  ...
  def visitChoiceChild(ctx: SchemaVisitorEx[DT, SF], choiceDefinition: RuntimeChildChoiceDefinition): Seq[SF] = {
  ...
  def visitChoiceChildOption(ctx: SchemaVisitorEx[DT, SF], choiceDefinition: RuntimeChildChoiceDefinition, optionName: String): Seq[SF] = {
    val definition = choiceDefinition.getChildByName(optionName)
    visitElement(ctx, choiceDefinition, definition, optionName)
  }

  def visitElementChild(ctx: SchemaVisitorEx[DT, SF], childDefinition: BaseRuntimeChildDefinition): Seq[SF] = {
  ...
  def visitElement(ctx: SchemaVisitorEx[DT, SF], childDefinition: BaseRuntimeChildDefinition,
