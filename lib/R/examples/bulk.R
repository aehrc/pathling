...
# Define the JWK and client_id for authentication
jwk <- '{
  "kty": "RSA",
  "alg": "RS384",
...
test_bulk_exports <- function() {
  ...
  pc %>% pathling_read_bulk(
    fhir_endpoint_url = fhir_server,
    output_dir = file.path(output_base, "system_detailed"),
    output_format = "application/fhir+ndjson",
    since = as.POSIXct("2015-01-01", tz = "UTC"),
    types = c("Patient", "Observation"),
    elements = c("id", "status"),
    include_associated_data = c("LatestProvenanceResources"),
    type_filters = c("Patient?status=active"),
  ...
  pc %>% pathling_read_bulk(
    fhir_endpoint_url = fhir_server,
    output_dir = file.path(output_base, "group_basic"),
    group_id = "BMCHealthNet"
  )
  cat("Group export completed successfully\n")

  # Test 3: Group level export with all parameters
  cat("\n=== Testing group level export with all parameters ===\n")
  pc %>% pathling_read_bulk(
    fhir_endpoint_url = fhir_server,
    output_dir = file.path(output_base, "group_detailed"),
    group_id = "BMCHealthNet",
    output_format = "application/fhir+ndjson",
    since = as.POSIXct("2015-01-01", tz = "UTC"),
  ...
  pc %>% pathling_read_bulk(
    ...
    output_format = "application/fhir+ndjson",
    since = as.POSIXct("2020-01-01", tz = "UTC"),
    types = c("Observation", "MedicationRequest"),
    elements = c("id", "status", "code"),
    include_associated_data = c("LatestProvenanceResources"),
    type_filters = c("Observation?category=vital-signs"),
    output_extension = "ndjson",
    timeout = 2400,
