...
def test_dfs_temp_dir(pathling_ctx):
    ...
    assert re.match(regex_pattern, temp_path), (
        f"Temp path {temp_path} does not match expected format"
    )
...
def test_dfs_operations(pathling_ctx):
    ...
    assert not dfs.exists(temp_path), (
        f"Temporary path {temp_path} should not exist before creation"
    )
    assert dfs.mkdirs(temp_path), f"Temporary path {temp_path} can be created"
    assert dfs.exists(temp_path), (
        f"Temporary path {temp_path} should exist after creation"
    )
    dfs.delete(temp_path, recursive=True)
    assert not dfs.exists(temp_path), (
        f"Temporary path {temp_path} should not exist after deletion"
    )
