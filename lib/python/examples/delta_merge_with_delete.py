...
from pathling import PathlingContext, SaveMode

HERE = os.path.abspath(os.path.dirname(__file__))

# Create Pathling context with Delta Lake enabled.
pc = PathlingContext.create(enable_delta=True)

# Read NDJSON data.
ndjson_dir = os.path.join(HERE, "data/resources/")
full_data = pc.read.ndjson(ndjson_dir)

# Create a temporary directory for the Delta tables.
temp_dir = tempfile.mkdtemp(prefix="delta_merge_delete_")

try:
    # Write the full dataset to Delta format.
    full_data.write.delta(temp_dir, save_mode=SaveMode.OVERWRITE)

    # Read back the data and count patients.
    initial_data = pc.read.delta(temp_dir)
    initial_count = initial_data.read("Patient").count()
    print(f"Initial patient count: {initial_count}")

    # Create a subset with only 3 specific patients.
    subset_patients = full_data.read("Patient").filter(
    ...
    subset_data.write.delta(temp_dir, save_mode=SaveMode.MERGE, delete_on_merge=True)

    # Read back the merged data and verify only 3 patients remain.
    merged_data = pc.read.delta(temp_dir)
    final_count = merged_data.read("Patient").count()
    print(f"Final patient count after merge with delete: {final_count}")
    print(f"Deleted {initial_count - final_count} patients")

    # Show the remaining patient IDs.
    print("\nRemaining patients:")
    merged_data.read("Patient").select("id").show()
