...
class PersistedRoundTripTest {
  ...
  /**
   * The dense schema carries every column the definitions describe, most of them null throughout,
   * which is the other shape Parquet has to preserve. Observation is used because its corpus
   * carries no extensions, which the dense bounds these tests are configured with do not store.
   */
  @Test
  void roundTripsSyntheaOnADenseSchema(@TempDir @Nonnull final Path directory) {
    final RoundTripHarness dense =
        RoundTripHarness.unconditional()
            .withConfiguration(SchemaConfiguration.builder().denseSchema(true).build());
    final Path corpus = corpus("synthea", "Observation");
    assertTrue(
        dense.findings("Observation", corpus).isEmpty(),
        "the corpus must be within the dense bounds, or a loss here is the bounds and not Parquet");

    dense.persistingTo(directory).assertRoundTrip("Observation", corpus);
  }
