 ...
 *
 * <p>The same enumeration serves the transform that writes the layout, so that the order a stored
 * structure is built in and the order a merge imposes cannot drift apart. They are the same list
 * read for different purposes: the transform takes the elements and the metadata groups from it,
 * and a merge takes the names.
 *
...
public final class DefinitionCanonicalStructure implements CanonicalStructure {
  ...
  /**
   * Returns the fields at this node, in canonical order, as the entries the transform needs rather
   * than as names alone.
   *
  ...
  public Optional<CanonicalStructure> field(@Nonnull final String name) {
    return entry(name)
        ...
        .or(() -> metadataGroup(name).map(CanonicalStructure.class::cast));
  }

  /**
   * Returns the entry for a named field, which is how the transform reaches the definition behind a
  ...
  public Optional<DefinitionCanonicalStructure> elementStructure(@Nonnull final LayoutEntry entry) {
    return entry
        .getElement()
        .filter(unused -> entry.primitiveType().isEmpty())
  ...
  public Optional<ElementDefinition> extensionElement() {
    // The entries are memoised, so finding one among them needs no memo of its own.
  ...
  private List<LayoutEntry> buildEntries() {
    ...
    for (final ChildDefinition child : node.getChildren()) {
      if (child instanceof final ChoiceDefinition choice) {
        ...
        choice.getAllChildTypes().forEach(variant -> addElement(result, variant));
  ...
  private void addElement(
      @Nonnull final List<LayoutEntry> result, @Nonnull final ElementDefinition element) {
    final String name = element.getElementName();
    if (resource && LayoutFields.CONTAINED.equals(name)) {
    ...
    result.add(LayoutEntry.element(name, element));
    if (PrimitiveTypes.isPrimitive(type.orElseThrow())) {
      result.add(LayoutEntry.metadataGroup(LayoutFields.metadataGroupName(name), element));
    }
    LayoutFields.annotationNames(name, type.orElseThrow())
        .forEach(annotation -> result.add(LayoutEntry.annotation(annotation, element)));
  }
}
