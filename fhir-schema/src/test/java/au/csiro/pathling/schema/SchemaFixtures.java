...
/**
 * Fixtures shared by the schema derivation tests: the R4 definitions, the bounds that apply to the
 * dense mode, and the hand-built structures that stand in for an inferred read schema.
 *
...
final class SchemaFixtures {
  ...
  /** Returns a builder for the dense and pruned modes, bounded as the defaults bound them. */
  @Nonnull
  static SchemaBuilder builder() {
    return SchemaBuilder.of(DEFINITIONS, 0, false, STANDARD_OPEN_TYPES);
  }

  /** Returns a builder with extensions enabled, for the cases that exercise them. */
  @Nonnull
  static SchemaBuilder builderWithExtensions() {
