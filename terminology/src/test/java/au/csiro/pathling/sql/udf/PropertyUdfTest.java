...
public class PropertyUdfTest extends AbstractTerminologyTestBase {
  ...
  static class Values<T> {
    ...
    public static <T> Values<T> ofPrimitive(
        final Function<T, Type> constructor, final List<T> values) {
      return of(values.stream().map(constructor).toList(), values);
    }
  }

  private static final Map<Class<?>, Values<?>> TEST_VALUES =
      ImmutableMap.of(
          // The last value is repeated for purpose for testing that
          // duplicate values are allowed in the result
          StringType.class,
          ofPrimitive(StringType::new, List.of("value_0", "value_1", "value_2", "value_2")),
          CodeType.class,
          ofPrimitive(CodeType::new, List.of("value_0", "value_1", "value_2", "value_2")),
          BooleanType.class,
          ofPrimitive(BooleanType::new, List.of(true, false, true, true)),
          IntegerType.class,
          ofPrimitive(IntegerType::new, List.of(1, 2, 3, 3)),
          DecimalType.class,
          ofPrimitive(
              DecimalType::new,
              List.of(
          ...
          DateTimeType.class,
          ofPrimitive(
              DateTimeType::new, List.of("2001-01-01", "2002-02-02", "2003-03-03", "2003-03-03")),
          Coding.class,
          Values.of(
              List.of(CODING_A, CODING_BB_VERSION1, CODING_C, CODING_C),
              List.of(
                  asArray(
                      List.of(CODING_A, CODING_BB_VERSION1, CODING_C, CODING_C)
                          .toArray(new Coding[] {})))));
  ...
  void testReturnsEmptyArrayWhenNonExisingProperty(final Class<? extends Type> udfClass) {
    setupUdf(udfClass);
    assertArrayEquals(
        new String[RANGE_ONE_FROM], propertyUdf.call(encode(CODING_A), "unknownProperty_0", null));
    assertArrayEquals(
        new String[RANGE_ONE_FROM],
    ...
    verify(terminologyService)
        .lookup(deepEq(CODING_BB_VERSION1), eq("unknownProperty_1"), eq(null));
    verifyNoMoreInteractions(terminologyService);
  }
  ...
  void testReturnsASingleValueForKnownProperty(final Class<? extends Type> udfClass) {
    ...
    setupLookup(terminologyService)
        .withProperty(
            CODING_A, "property_0", null, allTestValues().map(v -> v.fhirValueAt(0)).toList())
        .withProperty(
            CODING_BB_VERSION1,
            "property_1",
            null,
        ...
        .withProperty(
            CODING_A, "property_1", null, allTestValues().map(v -> v.fhirValueAt(2)).toList());
    ...
    assertArrayEquals(
        expectedSingletonOne.toArray(), propertyUdf.call(encode(CODING_A), "property_0", null));
    assertArrayEquals(
        expectedSingletonTwo.toArray(),
  ...
  void testReturnsManyValueForKnownProperty(final Class<? extends Type> udfClass) {
    ...
    setupLookup(terminologyService)
        ...
        .withProperty(
            CODING_B, "property_b", null, allTestValues().flatMap(Values::fhirRangeTwo).toList());
    ...
    assertArrayEquals(
        expectedArrayOne.toArray(),
        propertyUdf.call(encode(CODING_AA_VERSION1), "property_a", null));
    assertArrayEquals(
        expectedArrayTwo.toArray(), propertyUdf.call(encode(CODING_B), "property_b", null));
  }

  @ParameterizedTest
  @MethodSource("inputs")
  void testReturnsManyValueForKnownPropertyWithLanguage(final Class<? extends Type> udfClass) {
    setupUdf(udfClass);
    setupLookup(terminologyService)
        .withProperty(
            CODING_AA_VERSION1,
            ...
            allTestValues().flatMap(Values::fhirRangeOne).toList())
        .withProperty(
            CODING_B, "property_b", "fr", allTestValues().flatMap(Values::fhirRangeTwo).toList());
    ...
    assertArrayEquals(
        expectedArrayTwo.toArray(), propertyUdf.call(encode(CODING_B), "property_b", "fr"));
  }
}
