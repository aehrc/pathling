 ...
 *
 * <p>Wraps a Quantity struct Column and provides methods for:
 *
 * <ul>
 *   <li>Unit-based conversions (toQuantity, convertsToQuantity)
 *   <li>Quantity comparisons (equals, less than, greater than, etc.)
 *   <li>Accessing different representations (original, normalized, literal)
 * </ul>
 *
...
public class QuantityValue {
  ...
  @Nonnull private final Column quantityColumn;
  ...
  public record ValueWithUnit(
     ...
     *
     * <p>Uses canonicalized_value and canonicalized_code fields, with null system (as canonicalized
     ...
     *
     * <p>Uses value, code (canonical unit code), and system fields. This representation is used for
     ...
     *
     * <p>Uses value, unit (literal unit string as written, e.g., "days" instead of "day"), and
     * system fields. This representation is used for exact string matching in unit conversion.
     *
     * @param quantity the Quantity column
     ...
     *
     * <p>First checks that units match (and systems match if present), then applies the comparator
     * to the values. Returns null if units/systems don't match.
     *
     ...
     */
    @Nonnull
    public Column compare(
        @Nonnull final ValueWithUnit other, @Nonnull final BinaryOperator<Column> comparator) {
      ...
      return when(
          nonNull(system)
              ? requireNonNull(system)
                  .equalTo(requireNonNull(other.system))
   ...
   *
   * <p>Returns a ValueWithUnit with value, code (canonical unit), and system fields.
   *
   ...
   *
   * <p>Returns a ValueWithUnit with canonicalized_value, canonicalized_code, and null system.
   *
   ...
   *
   * <p>Returns a boolean column that evaluates to true if the quantity's system field equals {@link
   * UcumUnit#UCUM_SYSTEM_URI}, false otherwise.
   *
   ...
   *
   * <p>Returns a boolean column that evaluates to true if the quantity's system field equals {@link
   * CalendarDurationUnit#FHIRPATH_CALENDAR_DURATION_SYSTEM_URI}, false otherwise.
   *
   ...
   *
   * <p>First checks if this is a UCUM or calendar duration quantity. For exact unit match with
   * valid system, returns the quantity as-is (fast path). Otherwise, attempts UCUM unit conversion
   * via the UDF. Returns the converted quantity if conversion is successful, or null if conversion
   * is not possible.
   *
   * <p>Non-UCUM/non-calendar quantities (e.g., Money with system "urn:iso:std:iso:4217") will
   * always return null, even if the unit string happens to match the target unit.
   *
   * <p>This implements the FHIRPath toQuantity(unit) conversion semantics with full UCUM unit
   ...
   * conversion.
   *
   * <p>First checks if this is a UCUM or calendar duration quantity with exact unit match. If not,
   ...
   *
   * <p>Non-UCUM/non-calendar quantities (e.g., Money with system "urn:iso:std:iso:4217") will
   * always return false, even if the unit string happens to match the target unit.
   *
   * <p>This implements the FHIRPath convertsToQuantity(unit) validation semantics with full UCUM
