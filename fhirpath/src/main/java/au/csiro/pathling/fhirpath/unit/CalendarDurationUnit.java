 ...
 *
 * <p>Calendar duration units represent time periods as defined in the FHIRPath specification. These
 ...
 *   <li><b>Non-definite duration units</b>: year, month, week, day, hour, minute - have variable
 *       lengths (e.g., leap years, different month lengths) and cannot be reliably converted to
 *       other units in all contexts
 * </ul>
 *
 * <p>Each calendar duration unit has:
 *
 * <ul>
 *   <li>A canonical code (e.g., "year", "second")
 *   <li>A UCUM equivalent code (e.g., "a" for year, "s" for second)
 *   <li>A definite/non-definite classification
 *   <li>A millisecond conversion factor for unit conversions
 * </ul>
 *
 * <p>Calendar duration units support both singular and plural forms (e.g., "year" and "years").
 *
 * <p><b>IMPORTANT:</b> For non-definite duration units (year, month), the millisecond values are
 * <b>approximations</b> as specified by the FHIRPath specification:
 *
 * <ul>
 *   <li>1 year = 365 days (does not account for leap years)
...
public enum CalendarDurationUnit implements FhirPathUnit {
   ...
   *
   * <p>Millisecond calculation: 365 * 24 * 60 * 60 * 1000 (assumes 365 days/year, does NOT account
   * for leap years; approximation per FHIRPath spec)
   */
   ...
   * specification.
   *
   * <p>Millisecond calculation: 30 * 24 * 60 * 60 * 1000
   */
   ...
   *
   * <p>Millisecond calculation: 7 * 24 * 60 * 60 * 1000
   */
   ...
   *
   * <p>Millisecond calculation: 24 * 60 * 60 * 1000
   */
   ...
   *
   * <p>Millisecond calculation: 60 * 60 * 1000
   */
   ...
   *
   * <p>Millisecond calculation: 60 * 1000
   */
   ...
   *
   * <p>Millisecond calculation: 1000 ms
   */
   ...
   *
   * <p>Millisecond calculation: 1 ms
   */
  MILLISECOND("millisecond", true, "ms", BigDecimal.ONE);

  /** The system URI for FHIRPath calendar duration units (e.g. year, month, day). */
  public static final String FHIRPATH_CALENDAR_DURATION_SYSTEM_URI =
      "https://hl7.org/fhirpath/N1/calendar-duration";
   ...
   */
  @Getter private final boolean definite;

  /** The UCUM equivalent of the calendar duration unit. */
  @Getter @Nonnull private final String ucumEquivalent;
   ...
   */
  @Getter @Nonnull private final BigDecimal millisecondsEquivalent;
  ...
  CalendarDurationUnit(
      ...
      @Nonnull final String ucumEquivalent,
      @Nonnull final BigDecimal millisecondsEquivalent) {
    this.unit = code;
   ...
   *
   * @return the calendar duration system URI ({@value
  ...
  public boolean isValidName(@Nonnull final String unitName) {
    return fromString(unitName).map(this::equals).orElse(false);
  }
  ...
  public static CalendarDurationUnit parseString(@Nonnull final String name) {
    return fromString(name)
        .orElseThrow(() -> new IllegalArgumentException("Unknown calendar duration unit: " + name));
  }
  ...
  public Optional<BigDecimal> convertValue(
      @Nonnull final BigDecimal value, @Nonnull final CalendarDurationUnit targetUnit) {
   ...
   *
   * <p>This is a cross-type conversion that works by:
   *
   * <ol>
   *   <li>Converting this calendar unit value to seconds (if possible)
   *   <li>Converting the seconds value to the target UCUM unit
   * </ol>
   *
   * @param value the value to convert
  ...
  @Nonnull
  public Optional<BigDecimal> convertValue(
      @Nonnull final BigDecimal value, @Nonnull final UcumUnit ucumUnitTarget) {
    return convertValue(value, SECOND)
        .flatMap(secondsValue -> SECOND.asUcum().convertValue(secondsValue, ucumUnitTarget));
  }
   ...
   *
   * <ol>
   *   <li>Converting the UCUM source value to seconds ('s')
   *   <li>Converting the seconds value to this calendar unit
   * </ol>
   *
   * @param value the value to convert
   ...
   */
  @Nonnull
  public Optional<BigDecimal> convertValueFrom(
      @Nonnull final BigDecimal value, @Nonnull final UcumUnit ucumUnitSource) {
    return ucumUnitSource
        .convertValue(value, SECOND.asUcum())
        .flatMap(secondsValue -> SECOND.convertValue(secondsValue, this));
  }

  /**
   * Attempts to convert this calendar duration to its UCUM equivalent, if one exists.
   *
   * <p>Only definite duration units (second, millisecond) have UCUM equivalents because they have
    ...
    return Optional.ofNullable(SPECIAL_CASES.get(Pair.of(this, targetUnit)))
        .or(
            () ->
                ...
                Optional.of(
                    ConversionFactor.ofFraction(
                        getMillisecondsEquivalent(), targetUnit.getMillisecondsEquivalent())));
  }
   ...
   * weeks).
   */
  private static final Set<Pair<CalendarDurationUnit, CalendarDurationUnit>>
   ...
   * approximation).
   */
  private static final Map<Pair<CalendarDurationUnit, CalendarDurationUnit>, ConversionFactor>
      SPECIAL_CASES =
          Map.ofEntries(
