 ...
 * <p>
 * This collection is returned by the limited {@code resolve()} function and supports type checking
 * via the {@code is} operator, but does not allow traversal to child elements. Any attempt to
 * traverse will result in an {@link UnsupportedFhirPathFeatureError}.
 *
 * @author Piotr Szul
...
public class ResolvedReferenceCollection extends Collection {
   ...
   */
  protected ResolvedReferenceCollection(@Nonnull final ColumnRepresentation column,
   ...
   * <p>
   * The column should contain a value (typically a literal) that indicates the presence of a
  ...
  public static ResolvedReferenceCollection build(
    ...
    return new ResolvedReferenceCollection(
        column,
        fhirPathType,
        Optional.of(fhirType),
        Optional.empty(),
        Optional.empty());
  }

  /**
   * Builds a new ResolvedReferenceCollection from a column representation where the type is
   * encoded in the column itself.
   * <p>
   * The column should contain the string representation of the FHIR type (e.g., "Patient",
   ...
   * <p>
   * This method supports type checking with the {@code is} operator. If the resolved reference's
   * type matches the specified type, this collection is returned; otherwise, an empty collection
   * is returned.
   *
   ...
   * <p>
   * The {@code resolve()} function returns type information only, not actual resource data.
