 ...
 *
 * <p>This record represents a complete test specification that can contain multiple test cases for
...
public record YamlTestDefinition(
   ...
   *
   * <p>Each test case defines a FHIR Path expression to evaluate, along with the expected result or
   ...
   *
   * <p>Test cases support both positive testing (expecting specific results) and negative testing
   * (expecting errors). Error tests can specify exact error messages or use the {@link #ANY_ERROR}
   * constant to accept any error condition.
   *
  ...
  public record TestCase(
     ...
     *
     * <p>A test case is considered an error test if it has a non-null {@link #errorMsg}. Error
     * tests are used to validate that invalid expressions or edge cases properly fail with
     ...
     *
     * <p>A test case is considered parse-only if it has neither an expected error message nor an
   ...
   *
   * <p>This method handles the parsing of different YAML structures that can represent test cases.
   * It supports:
   *
   * <ul>
   *   <li>Direct test case objects with an "expression" field
   *   <li>Group objects containing lists of test cases
   *   <li>Multiple expressions within a single test case definition
   * </ul>
   *
   * <p>When a test case contains multiple expressions (as a list), each expression is converted
   * into a separate {@link TestCase} instance with the same configuration but different expression
   * values.
   *
   ...
   */
  @Nonnull
  static Stream<TestCase> mapCaseOrGroup(
          ...
          .map(
              expr ->
                  new TestCase(
                      ...
                      (boolean) caseOrGroup.computeIfAbsent("error", k -> false) ? ANY_ERROR : null,
    ...
    // Check if this is a group object (single key-value pair where value is a list).
      ...
      // If the single value is a list, recursively process it as nested test cases with the group
      // name.
   ...
   *
   * <p>YAML test specifications can define expressions in multiple formats:
   *
   * <ul>
   *   <li>Single string expression: "Patient.name"
   *   <li>List of expressions: ["Patient.name", "Patient.gender"]
   * </ul>
   *
   * This method normalizes these different formats into a consistent list of strings that can be
   * processed uniformly. If an unexpected object type is encountered, it logs a warning and returns
   * a failure expression for debugging purposes.
   *
   ...
   *
   * <p>This method helps with test identification in IDE test runners by including the group
   * context in the test description. The format is "groupName - description" when both are
   * available.
   *
   ...
   *
   * <p>This method processes the "tests" section of a YAML test specification, converting each
   * element into one or more {@link TestCase} instances. It handles nested structures and groups by
   * delegating to {@link #mapCaseOrGroup(Map)} for each individual case.
   *
   * <p>The input list typically comes directly from YAML parsing and may contain:
   *
   * <ul>
   *   <li>Individual test case maps
   *   <li>Group objects containing multiple test cases
   *   <li>Test cases with multiple expressions
   * </ul>
   *
   ...
   * context.
   *
   * <p>This overloaded method is used when processing test cases within a group. The group name is
   ...
   *
   * <p>This is the primary factory method for creating test specifications from YAML files or
   * strings. The YAML structure should follow the expected format with optional "subject" and
