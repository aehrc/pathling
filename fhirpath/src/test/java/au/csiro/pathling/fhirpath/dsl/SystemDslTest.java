...
public class SystemDslTest extends FhirPathDslTestBase {
  ...
  public Stream<DynamicTest> testLiterals() {
    return builder()
        .withSubject(sb -> sb.string("stringValue", "test"))
        ...
        .testEquals(
            "http://example.org", "(http://example.org|code).system", "Coding literal system")
        .testEquals("code", "(http://example.org|code).code", "Coding literal code")
        .testEquals(
            "display", "(http://example.org|code||display).display", "Coding literal display")
        .testEquals(
            true,
            "(http://example.org|code|v1|display|true).userSelected",
        ...
        .testEquals(
            "carriage\rreturn", "'carriage\\rreturn'", "String literal with carriage return")
        ...
        .testEquals(
            "Unicode: Peter", "'Unicode: P\\u0065ter'", "String literal with Unicode escape")
        .testEquals(
            "All escapes: \\\r\n\t\f\"`'",
            "'All escapes: \\\\\\r\\n\\t\\f\\\"`\\''",
            "String literal with all escape sequences")
        .group("Date and time literals")
        .testEquals("2019-02-04", "@2019-02-04", "Date literal")
        .testEquals("2019-02-04T14", "@2019-02-04T14", "DateTime literal with hours precision")
        .testEquals(
            "2019-02-04T14:34:28+09:00",
            "@2019-02-04T14:34:28+09:00",
        ...
        .group("Quantity literals")
        .testTrue("(10 'mg').exists()", "Quantity literal is  supported")
        .testTrue("(4 days).exists()", "Time quantity literal is supported")
        .build();
  }
  ...
  public Stream<DynamicTest> testBooleanEvaluation() {
    return builder()
        ...
        .testFalse(
            "choiceField.value.not()", "single polymorphic choice evaluates to true in function")
        .testFalse(
        ...
        .testTrue(
            "trueValue and true", "single true boolean evaluates to true in boolean operators")
        .testFalse(
            "falseValue or false", "single true boolean evaluates to true in boolean operators")
        ...
        .testTrue(
            "{} or stringArrayOne", "array of one String evaluates to true in boolean operators")
        .testTrue(
        ...
        .testError(
            "booleanArray.not()", "array of booleans evaluates to error as boolean singleton")
        .testError(
            "stringArray.not()", "array of many strings evaluates to error as boolean singleton")
        .testError(
        ...
        .testError(
            "booleanArray and true", "array of booleans evaluates to error in boolean operators")
        .testError(
  ...
  public Stream<DynamicTest> testPathTraversal() {
    return builder()
        ...
        .testEmpty("unknown", "traversal to undefined property returns {}")
        .testTrue("trueValue", "correct traversal to a 'true' boolean property")
        .testFalse("falseValue", "correct traversal to a 'false' boolean property")
        .group("traversal to complex literal types")
        .testEquals(
            "urn:system1",
            "(urn:system1|code1).system",
            "correct traversal to a Coding literal system")
        .testEquals(
            "code2", "(urn:system2|code2).code", "correct traversal to a Coding literal code")
        .testEquals(
            "display3",
            "(urn:system2|code2||display3).display",
            "correct traversal to a Coding literal display")
        .testEquals(
            true,
            "(urn:system2|code2|v1|display|true).userSelected",
  ...
  public Stream<DynamicTest> testPathTraversalOnFhirResource() {
    ...
    final Resource observation = new Observation().setValue(new StringType("testValue")).setId("1");

    return builder()
        .withResource(observation)
        .group("basic traversal of FHIR resource")
        .testEmpty("unknown", "traversal to undefined property returns {}")
        .testEquals("1", "id", "correct traversal an ID property")
        .testEquals(
            "testValue",
            "value.ofType(string)",
        ...
        .testEmpty(
            "valueInteger", "correct direct traversal to an missing choice value in FHIR resource")
        .group("choice type error handling")
        .testError(
            "Selection of mixed collection not supported: value",
            "value",
