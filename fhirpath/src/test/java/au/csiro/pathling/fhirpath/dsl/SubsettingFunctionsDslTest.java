...
public class SubsettingFunctionsDslTest extends FhirPathDslTestBase {
  ...
  public Stream<DynamicTest> testFirst() {
    return builder()
        ...
        // Basic first() tests
        .testEquals("test", "singleString.first()", "first() returns the single string value")
        .testEquals(
            "one", "stringArray.first()", "first() returns the first item in a string array")
        .testEmpty("emptyString.first()", "first() returns empty for an empty string")
        .testEquals(42, "singleInteger.first()", "first() returns the single integer value")
        .testEquals(true, "singleBoolean.first()", "first() returns the single boolean value")
        ...
        .testTrue(
            "people.first().name = 'Alice'",
            "first() returns the first item in a complex type array with expected name")
        .testEmpty("emptyComplex.first()", "first() returns empty for an empty complex type")
        .testEmpty("{}.first()", "first() returns empty for an empty literal")

        // Chained first() tests
        .testEquals("Alice", "people.first().name", "first() can be chained with property access")
        .testEquals(
            25, "people.first().age", "first() can be chained with property access for integer")
        .build();
  }
}
