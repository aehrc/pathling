 ...
 * <p>
 * The {@code resolve()} function returns type information for references, supporting type checking
 * with the {@code is} operator. It does not support field traversal.
 * <p>
 * Type extraction behavior:
 * <ul>
 *   <li>If {@code Reference.type} field is present, it is used regardless of reference format
 *       (including contained and logical references)</li>
 *   <li>If {@code Reference.type} is absent, type is parsed from {@code Reference.reference}
 *       for relative, absolute, and canonical reference formats</li>
 *   <li>The {@code type} field always takes precedence over the parsed type</li>
 * </ul>
 *
 * @author Piotr Szul
...
public class ResolveFunctionDslTest extends FhirPathDslTestBase {
  ...
  public Stream<DynamicTest> testTypePriority() {
    return builder()
        .withSubject(sb -> sb
            ...
            .element("conflictRef", ref -> ref
                .fhirReference()
                .string("reference", "Patient/123")
                .string("type", "Practitioner"))

            // Type field present, no reference string
            .element("typeOnlyRef", ref -> ref
                .fhirReference()
                .string("type", "Organization"))

            // Reference string only, no type field
            .element("refOnlyRef", ref -> ref
                .fhirReference()
                .string("reference", "Encounter/456"))

            .element("containedWithType", ref -> ref
                .fhirReference()
                .string("type", "Observation")
                .string("reference", "#Test"))
        )
        .group("Type priority - type field takes precedence")
        .testTrue("conflictRef.resolve() is Practitioner",
            "Type field takes precedence over reference string")
        .testFalse("conflictRef.resolve() is Patient",
            "Reference string type is ignored when type field present")
        .testTrue("typeOnlyRef.resolve() is Organization",
            "Reference with only type field resolves correctly")
        .testTrue("refOnlyRef.resolve() is Encounter",
            "Reference with only reference string resolves correctly")
        .testTrue("containedWithType.resolve() is Observation",
  ...
  public Stream<DynamicTest> testEdgeCases() {
    return builder()
        .withSubject(sb -> sb
            ...
            .element("containedRef", ref -> ref
                .fhirReference()
                .string("reference", "#patient-1"))

            // Logical reference (identifier-only) WITH type field
            .element("logicalWithType", ref -> ref
                .fhirReference()
                .string("type", "Patient"))
            ...
            // Malformed reference
            .element("badRef", ref -> ref
                .fhirReference()
                .string("reference", "not-a-valid-reference"))

            // Reference with just an ID (no resource type)
            .element("idOnlyRef", ref -> ref
                .fhirReference()
                .string("reference", "12345"))

            // Reference that's just a #
            .element("hashOnlyRef", ref -> ref
                .fhirReference()
                .string("reference", "#"))

            // Reference with lowercase type (invalid)
            .element("lowercaseRef", ref -> ref
                .fhirReference()
                .string("reference", "patient/123"))
        )
        .group("Edge cases - contained reference without type")
        .testEmpty("containedRef.resolve()",
            "Contained reference without type field returns empty collection")

        .group("Edge cases - logical references")
        .testTrue("logicalWithType.resolve() is Patient",
            "Logical reference with type field resolves correctly")
        .testEmpty("logicalNoType.resolve()",
            "Logical reference without type field returns empty collection")

        .group("Edge cases - empty and malformed")
        .testEmpty("emptyRef.resolve()",
            "Empty reference returns empty collection")
        .testEmpty("badRef.resolve()",
            "Malformed reference returns empty collection")
        .testEmpty("idOnlyRef.resolve()",
            "Reference with just ID returns empty collection")
        .testEmpty("hashOnlyRef.resolve()",
            "Reference with just # returns empty collection")
        .testEmpty("lowercaseRef.resolve()",
            "Reference with lowercase type returns empty collection")
  ...
  public Stream<DynamicTest> testTraversalPrevention() {
    return builder()
        ...
        .testError("patientRef.resolve().name",
            "Traversal after resolve() is not supported")
        .testError("patientRef.resolve().id",
            "Cannot access id field after resolve()")
        .testError("patientRef.resolve().managingOrganization",
            "Cannot access reference field after resolve()")
        .testError("patientRef.resolve().active",
            "Cannot access boolean field after resolve()")
  ...
  public Stream<DynamicTest> testInputValidation() {
    return builder()
        ...
        .testError("stringValue.resolve()",
            "resolve() on string throws InvalidUserInputError")
        .testError("integerValue.resolve()",
            "resolve() on integer throws InvalidUserInputError")

        .group("Input validation - valid reference input")
        .testTrue("patientRef.resolve() is Patient",
            "resolve() works correctly on valid Reference type")
  ...
  public Stream<DynamicTest> testQualifiedTypeNames() {
    return builder()
        ...
        .testTrue("patientRef.resolve() is FHIR.Patient",
            "Type checking works with FHIR namespace qualifier")

        .group("Qualified type names - unqualified")
        .testTrue("patientRef.resolve() is Patient",
            "Type checking works with unqualified type name")
  ...
  public Stream<DynamicTest> testVariousReferenceFormats() {
    return builder()
        .withSubject(sb -> sb
            ...
            .element("relativeRef", ref -> ref
                .fhirReference()
                .string("reference", "Patient/123"))

            .element("absoluteRef", ref -> ref
                .fhirReference()
                .string("reference", "http://example.org/fhir/Patient/456"))

            .element("absoluteRefHttps", ref -> ref
                .fhirReference()
                .string("reference", "https://example.org/fhir/Practitioner/789"))

            .element("canonicalSimple", ref -> ref
                .fhirReference()
                .string("reference", "http://hl7.org/fhir/ValueSet/example"))

            .element("canonicalWithVersion", ref -> ref
                .fhirReference()
                .string("reference", "http://hl7.org/fhir/CodeSystem/example|2.0.0"))

            // URN format (should return empty)
            .element("urnRef", ref -> ref
                .fhirReference()
                .string("reference", "urn:uuid:53fefa32-fcbb-4ff8-8a92-55ee120877b7"))
        )
        .group("Various reference formats - relative")
        .testTrue("relativeRef.resolve() is Patient",
            "Relative reference format resolves correctly")

        .group("Various reference formats - absolute HTTP")
        .testTrue("absoluteRef.resolve() is Patient",
            "Absolute HTTP reference resolves correctly")
        .testTrue("absoluteRefHttps.resolve() is Practitioner",
            "Absolute HTTPS reference resolves correctly")

        .group("Various reference formats - canonical")
        .testTrue("canonicalSimple.resolve() is ValueSet",
            "Canonical URL resolves correctly")
        .testTrue("canonicalWithVersion.resolve() is CodeSystem",
            "Canonical URL with version resolves correctly")

        .group("Various reference formats - URN (unsupported)")
        .testEmpty("urnRef.resolve()",
            "URN reference returns empty collection")
  ...
  public Stream<DynamicTest> testResolveCollections() {
    return builder()
        .withSubject(sb -> sb
            ...
            .elementArray("mixedValidInvalid",
                ref1 -> ref1
                    .fhirReference()
                ...
                ref4 -> ref4
                    .fhirReference()
                    .string("display", "invalid-empty")  // Empty reference and type - should not resolve
            )
        )
        .group("Resolve collections - all same type")
        .testTrue("patientRefs.resolve().first() is Patient",
            "First resolved reference is Patient type")
        .testTrue("patientRefs.resolve()[0] is Patient",
            "First resolved reference (by index) is Patient type")
        .testTrue("patientRefs.resolve()[1] is Patient",
            "Second resolved reference (by index) is Patient type")
        .testTrue("patientRefs.resolve()[2] is Patient",
            "Third resolved reference (by index) is Patient type")

        .group("Resolve collections - mixed types with ofType()")
        .testTrue("mixedRefs.resolve().ofType(Patient).exists()",
            "Can filter resolved references to Patient type")
        .testTrue("mixedRefs.resolve().ofType(Practitioner).exists()",
            "Can filter resolved references to Practitioner type")
        .testTrue("mixedRefs.resolve().ofType(Location).exists()",
            "Can filter resolved references to Location type")
        .testFalse("mixedRefs.resolve().ofType(Organization).exists()",
            "Filtering to non-existent type returns no results")

        .group("Resolve collections - first() and is/as operators")
        .testTrue("mixedRefs.resolve().first() is Patient",
            "First resolved reference is Patient")
        .testTrue("mixedRefs.resolve()[1] is Practitioner",
            "Second resolved reference is Practitioner")
        .testTrue("mixedRefs.resolve()[2] is Location",
            "Third resolved reference is Location")

        .group("Resolve collections - counting and filtering")
        .testEquals(3, "patientRefs.resolve().count()",
            "Resolving 3 Patient references returns 3 items")
        .testEquals(4, "mixedRefs.resolve().count()",
            "Resolving 4 mixed references returns 4 items")
        .testEquals(List.of("mixed-p1", "mixed-p2"),
            "mixedRefs.where(resolve() is Patient).display",
            "Filtering mixed refs by Patient type returns correct displays")
        .testEquals(List.of("mixed-pr1"),
        ...
        .testEquals(2, "mixedValidInvalid.resolve().count()",
            "Invalid references are excluded from count")
        .testEquals(1, "mixedValidInvalid.resolve().ofType(Patient).count()",
            "One Patient reference after filtering out invalid refs")
        .testEquals(1, "mixedValidInvalid.resolve().ofType(Practitioner).count()",
  ...
  public Stream<DynamicTest> testResolveWithHapiResources() {
    ...
    return builder()
        ...
        .testTrue("Patient.generalPractitioner.resolve().first() is Practitioner",
  ...
  public Stream<DynamicTest> testResolveWithAsOperator() {
    return builder()
        .withSubject(sb -> sb
            ...
            .element("practitionerRef", ref -> ref
                .fhirReference()
        ...
        .testTrue("patientRef.resolve().as(Patient).exists()",
            "Can use as() operator after resolve() for matching type")
        .testEmpty("patientRef.resolve().as(Practitioner)",
            "as() returns empty for non-matching type")
        .testEmpty("practitionerRef.resolve().as(Patient)",
            "as() returns empty when type doesn't match")

        .group("resolve() with as operator - collections")
        .testTrue("mixedRefs.resolve().first().as(Patient).exists()",
            "Can use as() on first() of resolved collection")
        .testTrue("mixedRefs.resolve()[1].as(Practitioner).exists()",
