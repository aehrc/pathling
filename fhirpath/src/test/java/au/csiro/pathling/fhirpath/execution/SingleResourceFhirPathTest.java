...
class SingleResourceFhirPathTest {
  ...
  CollectionDataset evalExpression(
    ...
    return createEvaluator(dataSource).evaluate(subjectResourceCode, fhirExpression).toCanonical();
  }
  ...
  FhirPathExecutor createEvaluator(@Nonnull final DataSource datasource) {
    return FhirPathExecutor.of(
        new Parser(),
        new SingleEvaluatorProvider(
            encoders.getContext(), StaticFunctionRegistry.getInstance(), Map.of(), datasource));
  }
  ...
  void nullHandlingTests() {
    ...
    final Dataset<Row> result = selectExpression(dataSource, "Patient", "name.given");
  ...
  void resourceExtensionTest() {
    ...
    final Dataset<Row> resultDataset =
        selectExpression(dataSource, "Patient", "extension('urn:ex1').value.ofType(string)");
  ...
  void nestedExtensionTraversalTest() {
    final ObjectDataSource dataSource = getExtensionTestSource();
    final Dataset<Row> resultDataset =
        selectExpression(dataSource, "Patient", "extension.extension.url");
    new DatasetAssert(resultDataset)
        .hasRowsUnordered(
  ...
  void elementExtensionTest() {
    ...
    final Dataset<Row> resultDataset =
        selectExpression(dataSource, "Patient", "name.extension('urn:name1').value.ofType(string)");
    new DatasetAssert(resultDataset)
        .hasRowsUnordered(
  ...
  void ofTypeExtensionTest() {
    ...
    final Dataset<Row> resultDataset =
        selectExpression(dataSource, "Patient", "extension('urn:ex2').value.ofType(integer)");
    new DatasetAssert(resultDataset)
        .hasRowsUnordered(
            RowFactory.create("1", null),
  ...
  void testOfTypeForChoice() {
    ...
    final CollectionDataset evalResult =
        evalExpression(dataSource, "Observation", "value.ofType(integer)");
  ...
  void testOfTypeForComplexChoice() {
    ...
    final CollectionDataset evalResult =
        evalExpression(dataSource, "Observation", "component.value.ofType(Quantity).value");
  ...
  void testFHIRTypeOfIntegerMathOperation() {
    ...
    final CollectionDataset evalResult = evalExpression(dataSource, "Coverage", "order - 11");
  ...
  void testOfTypeForReference() {
    ...
    final CollectionDataset evalResult =
        evalExpression(dataSource, "Observation", "extension.value.ofType(Reference).reference");
  ...
  void testComparisonBase64Binary() {
    ...
    final CollectionDataset evalResult =
        evalExpression(dataSource, "Device", "udiCarrier.carrierAIDC = 'QUlEMQ=='");
