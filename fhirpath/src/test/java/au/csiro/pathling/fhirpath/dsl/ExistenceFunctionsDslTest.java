...
public class ExistenceFunctionsDslTest extends FhirPathDslTestBase {
  ...
  public Stream<DynamicTest> testCount() {
    return builder()
        ...
        .testEquals(1, "stringArray.where($this = 'one').count()",
            "returns 1 after filtering to single match")
        .testEquals(0, "stringArray.where($this = 'four').count()",
            "returns 0 after filtering to no matches")
        // Filtered complex array
        .testEquals(2, "people.where(active = true).count()",
            "returns 2 for filtered complex type array")
  ...
  public Stream<DynamicTest> testCountOnFhirResource() {
    ...
    return builder()
        ...
        .testEquals(2, "Patient.name.given.count()", "returns 2 for Patient.name.given (nested array)")
        ...
        .testEquals(1, "Patient.telecom.where(system = 'phone').count()",
            "returns 1 for filtered Patient.telecom by phone")
        .testEquals(1, "Patient.telecom.where(system = 'email').count()",
            "returns 1 for filtered Patient.telecom by email")
        .testEquals(0, "Patient.telecom.where(system = 'fax').count()",
        ...
        .testTrue("Patient.telecom.count() > 1", "count of Patient.telecom in greater than comparison")
        .testFalse("Patient.address.count() > 0", "count of empty Patient.address is not greater than 0")
        .build();
  }
}
