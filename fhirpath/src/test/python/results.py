...
def convert_message(message):
    ...
    msg1 = re.sub(
        r"Error parsing FHIRPath expression \(line: \d+, position: \d+\)",
        "Error parsing FHIRPath expression (line: ??, position: ??)",
        message,
    )
...
def to_results(tree: ET.ElementTree) -> List[TestResult]:
    ...
    for testcase in root.findall("testcase"):
        ...
        if failure is not None:
            ...
            desc = (
                EX_DESC.search(sysout.text).group(1)
                if EX_DESC.search(sysout.text)
                else None
            )
            expr = (
                EX_EXPR.search(sysout.text).group(1)
                if EX_EXPR.search(sysout.text)
                else None
            )
        ...
        if error is not None:
            ...
            yield TestResult(
                name,
                error.get("type") + ": " + convert_message(error.get("message")),
                tag,
                rule,
                desc,
                expr,
