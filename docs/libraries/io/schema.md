...
This specification describes a scheme for representing FHIR resources within a [Parquet](https://parquet.apache.org/) schema.
...
## Resource type support[​](#resource-type-support "Direct link to Resource type support")
...
### Primitive elements[​](#primitive-elements "Direct link to Primitive elements")
...
### Complex and backbone elements[​](#complex-and-backbone-elements "Direct link to Complex and backbone elements")
...
Each field SHALL be represented as described in the [Primitive elements](#primitive-elements) and [Choice types](#choice-types) sections (or this section in the case of a nested complex or backbone element).
...
### Choice types[​](#choice-types "Direct link to Choice types")
...
### Decimal type[​](#decimal-type "Direct link to Decimal type")
...
A value with more than 6 decimal places is stored in the `DECIMAL` field rounded to a scale of 6, while the `_scale` field retains the scale of the original FHIR data. A `_scale` value greater than 6 therefore identifies a value that lost precision during encoding.

### ID type[​](#id-type "Direct link to ID type")
...
### Quantity type[​](#quantity-type "Direct link to Quantity type")

If a complex element is of type [Quantity](https://hl7.org/fhir/R4/datatypes.html#Quantity), an additional two fields SHALL be included as part of the group. These fields SHALL be named `_value_canonicalized` and `_code_canonicalized`.
...
### Reference type[​](#reference-type "Direct link to Reference type")

If a complex element is of type [Reference](https://hl7.org/fhir/R4/references.html#Reference), it is represented as described in [Complex and backbone elements](#complex-and-backbone-elements) with the following exception:

* The `assigner` field within the `identifier` field within a `Reference` SHALL NOT be included in the schema.
...
The group used for each value in the map SHALL be represented using the [Extension](https://hl7.org/fhir/R4/extensibility.html#extension) type, as described in [Complex and backbone elements](#complex-and-backbone-elements).
...
The following schema is an example of how to represent the [Patient](https://hl7.org/fhir/R4/patient.html) resource type using the following configuration values:
