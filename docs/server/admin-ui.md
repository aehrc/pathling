...
When [authorization](/docs/server/authorization.md) is enabled (`pathling.auth.enabled: true`), the admin UI requires authentication via SMART on FHIR. Users must log in before accessing protected features.
...
The `pathling` authority grants access to all operations and resources. See [Authorization](/docs/server/authorization.md#authorities) for more granular authority options.
