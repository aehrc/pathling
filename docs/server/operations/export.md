...
The export operation allows FHIR data to be exported from Pathling in bulk. This implementation follows the [FHIR Bulk Data Access specification](https://build.fhir.org/ig/HL7/bulk-data/export.html).

## Export levels[​](#export-levels "Direct link to Export levels")
...
The export operation uses the [FHIR Asynchronous Request Pattern](https://hl7.org/fhir/R4/async.html). Include a `Prefer: respond-async` header to initiate asynchronous processing.
...
### Export client[​](#export-client "Direct link to Export client")
