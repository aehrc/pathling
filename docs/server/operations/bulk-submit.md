...
| `oauthMetadataUrl`    | 0..1        | string     | Explicit URL to OAuth 2.0 metadata. If not provided, SMART discovery is used on `fhirBaseUrl`. Must match a prefix in `pathling.bulkSubmit.allowableSources`. |
...
The `allowableSources` configuration is mandatory: when no prefixes have been configured every URL is rejected and the operation cannot be used. It applies to **every URL fetched** during `$bulk-submit` processing, including the `manifestUrl` provided in the request, every `output[].url` discovered within fetched manifests, and the `oauthMetadataUrl` parameter when supplied. A manifest served from an allowlisted source that references file URLs on a different host will be rejected before any file download is attempted.
