...
1. **Submit manifests** — The Data Provider sends one or more requests with
   `submissionStatus: in-progress` and provides the `manifestUrl` pointing to
   bulk data manifests. Pathling downloads the files in the background.
2. **Mark complete** — Once all manifests have been submitted and downloads are
   finished, the Data Provider sends a request with `submissionStatus: complete`
...
sequenceDiagram
    ...
    PS-->>DP: 200 OK (downloading)
    DP->>PS: POST /$bulk-submit (in-progress + manifestUrl)
    PS-->>DP: 200 OK (downloading)
    DP->>PS: POST /$bulk-submit (complete)
    PS-->>DP: 200 OK (import started)
    DP->>PS: POST /$bulk-submit-status
    PS-->>DP: 202 Accepted (X-Progress: 50%)
    DP->>PS: POST /$bulk-submit-status
    PS-->>DP: 200 OK (manifest)
```
