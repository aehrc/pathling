---
sidebar_position: 5
description: Pathling supports both standard FHIR search parameters and FHIRPath-based search for filtering resources.
---
...
1. **Standard FHIR search parameters** — filter using the search parameters
   defined in the FHIR specification (e.g., `gender`, `birthdate`, `code`).
2. **FHIRPath-based search** — filter using arbitrary
   [FHIRPath](/docs/fhirpath) expressions for more complex queries.

Both approaches can be combined in a single request.

## Standard search parameters

Standard FHIR search parameters can be used directly in the URL query string,
following the [FHIR search specification](https://hl7.org/fhir/R4/search.html).
...
- **AND logic**: Repeat the same or different parameters
  (`gender=male&active=true`).
- **OR logic**: Use comma-separated values within a single parameter
  (`gender=male,female`).
...
## FHIRPath-based search

For queries that go beyond what standard search parameters can express, Pathling
supports FHIRPath-based search using a
[named query](https://hl7.org/fhir/R4/search.html#advanced) called `fhirPath`.

Each FHIRPath expression is evaluated against each resource, returning a Boolean
...
Multiple instances of the `filter` parameter are combined using Boolean AND
...
[FHIRPath boolean operators](https://hl7.org/fhirpath/#boolean-logic) can be
...
Retrieve female patients who are active (AND logic via multiple `filter`
parameters):
...
This is the URL-encoded form of
`code.coding.code='8867-4' and valueQuantity.value > 100`.

## Combining standard and FHIRPath search

Standard search parameters can be used alongside FHIRPath filters in the same
request by including `_query=fhirPath` with both `filter` and standard
...
This retrieves active male patients using a FHIRPath filter for `active` and a
standard search parameter for `gender`.
...
The following errors return HTTP 400 Bad Request with an `OperationOutcome`
resource:

- **Unknown search parameters**: Using a parameter not defined for the resource
  type (e.g., `Patient?nonexistent=value`).
- **Unsupported modifiers**: Using a modifier not supported for the parameter
  type (e.g., `gender:exact=male` — `:exact` is only valid for string
  parameters).
- **Invalid FHIRPath expressions**: Syntax errors in FHIRPath filter
  expressions.
...
- Underscore-prefixed common search parameters: `_id`, `_lastUpdated`, `_tag`,
  `_profile`, `_security`, `_text`, `_content`, `_list`, `_has`, `_type`.
