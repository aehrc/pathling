...
public class EpcarToFhirTransformer {
  ...
  private static final String NS_PROCEDURE_REQUEST_ID =
      NAMESPACE_PREFIX + "/epcar-procedure-request-id";
  private static final String NS_DIAGNOSTIC_REPORT_ID =
      NAMESPACE_PREFIX + "/epcar-diagnostic-report-id";
  ...
  public Bundle transform(String message) throws Exception {
    ...
    logger.info(
        "Converted HL7 v2.4 pathology report in " + String.format("%.1f", elapsedSecs) + " ms");
    return bundle;
  }
  ...
  private void transformOrderObservation(Bundle bundle, Encounter encounter,
    ...
    if (!diagnosticReportIdentifier.isPresent()) {
      throw new Exception("Unable to retrieve identifier from " +
          "DiagnosticReport");
    }
    ...
    for (ORU_R01_OBSERVATION observation : observationAll) {
      ...
      Observation fhirObservation = extractObservation(obx,
          diagnosticReportIdentifier.get().getValue());
  ...
  /**
   * Processes the Patient Visit (PV1) part of the HL7 v2 message, and returns an Encounter
   * resource.
   */
  private Encounter extractEncounter(PV1 pv1) throws HL7Exception {
    ...
    Optional<Parameters.ParametersParameterComponent> result =
        translateResult.getParameter().stream().filter(param -> param.getName().equals("result"))
            .findFirst();
    boolean resultValue =
    ...
    if (resultValue) {
      Optional<Parameters.ParametersParameterComponent> selectedMatch = selectTranslation(
          matches);
      if (selectedMatch.isPresent()) {
        ...
        if (concept.isPresent() && concept.get().getValue() instanceof Coding) {
          ...
          logger
              .warn(
                  "Unable to get concept value from match parameter in translate response");
        }
      } else {
        logger.warn("Unable to select match from translate response");
      }
    } else {
      logger.warn("Result parameter not found within translate response");
    }
    ...
    encounter.getLocation()
        .add(new Encounter.EncounterLocationComponent(pointOfCareLocationReference));

    return encounter;
  }

  /**
   * Processes the Observation Request (OBR) part of the HL7 v2 message, and returns a
  ...
  private DiagnosticReport extractDiagnosticReport(OBR obr) throws DataTypeException {
    ...
    Date observationEndDateTime = obr.getObservationEndDateTime().getTimeOfAnEvent()
        .getValueAsDate();
   ...
   */
  private Observation extractObservation(OBX obx, String diagnosticReportId)
    ...
    String alternateSystem = obx.getObservationIdentifier().getNameOfAlternateCodingSystem()
        .getValue();
   ...
   * <p>
   * Currently the way this works is that only `equivalent`, `wider` and `narrower` matches are
   * accepted, and selected using this order of preference.
   */
  private Optional<Parameters.ParametersParameterComponent> selectTranslation(
    ...
    Optional<Parameters.ParametersParameterComponent> equivalentMatch = getMatchWithEquivalence(
        matches,
        "equivalent");
    ...
    Optional<Parameters.ParametersParameterComponent> widerMatch = getMatchWithEquivalence(matches,
        "wider");
    if (widerMatch.isPresent()) {
      return widerMatch;
    }
  ...
  /**
   * Takes a list of translate matches, and returns the first one with the matching `equivalence`
   * value.
   */
