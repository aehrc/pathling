...
The R library SHALL provide an exported function `pc_fhirpath_to_column(pc, resource_type, fhirpath_expression)` that invokes the Java `PathlingContext.fhirPathToColumn()` method via sparklyr's `j_invoke` and returns a `spark_jobj` representing a Spark Column.

#### Scenario: Boolean expression returns a Column for filtering

- **WHEN** `pc %>% pc_fhirpath_to_column("Patient", "gender = 'male'")` is called
...
- **WHEN** `pc %>% pc_fhirpath_to_column("Patient", "name.given.first()")` is called
...
- **WHEN** `pc %>% pc_fhirpath_to_column("Patient", "!!invalid!!")` is called
