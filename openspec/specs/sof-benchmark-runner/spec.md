...
`benchmark-report.schema.json` (contract v2), including:
...
- a `results` map keyed by the authored suite `name`, each entry carrying `size`,
  ...
  with `status`, `inputRows`, `outputRows`, timing `samplesMs`, and a `stats` object of
  exactly `{ mean, stddev, min, max, median }`.
