...
/**
 * Describes a dimension, which is a set of related data fields that are available for grouping and filtering within a
...
public class Dimension extends DomainResource {
    ...
    /**
     * Generate the JSON representation of this resource, using the supplied parser, and put it into the `json`
     * field, ready for persistence.
     *
    ...
    public void copyValues(DomainResource dst) {
        ...
        if (attribute != null) {
            ((Dimension) dst).attribute = attribute.stream().map(Reference::copy).collect(Collectors.toList());
        }
        if (describes != null) {
            ((Dimension) dst).describes = describes.stream().map(DescribesComponent::copy).collect(Collectors.toList());
        }
        if (json != null) {
    ...
    protected void listChildren(List<Property> children) {
        ...
        children.add(new Property("title", "string", "The machine-friendly name of the Dimension.", 1, 1, title));
    ...
    public static class DescribesComponent extends BackboneElement {
        ...
        public String getStructureDefinition() {
            if (structureDefinition == null) return null;
