...
public class PathResolver {
  ...
  public static PathTraversal resolvePath(@Nonnull String path)
    ...
    PathTraversal result = new PathTraversal(path);
    ...
    while (true) {
      ...
      harvestMultiValueTraversal(result, currentPath, elementDefinition);
  ...
  private static void harvestMultiValueTraversal(
    ...
    if (!element.getMaxCardinality().equals("1")) {
      ...
      if (isComplex(element.getTypeCode())) {
        ElementDefinition complexElement = getElementsForType(element.getTypeCode())
