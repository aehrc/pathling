...
public interface SqlExtensions {
  ...
  /**
   * Creates a new datasets with the additional column as specified in `resultField` with the
   * results of mapping each of the rows in the input dataset. The mapping extracts the value
   * specified input column from the input dataset and the uses the provided mapper to a) create a
   * state for each partition b) to map the values for each row given access to per partition
   * state.
   *
