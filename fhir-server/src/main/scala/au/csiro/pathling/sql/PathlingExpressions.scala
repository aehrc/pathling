 ...
 */
case class Normalize(child: Expression) extends UnaryExpression with CodegenFallback with NullIntolerant {
  ...
  override protected def withNewChildInternal(newChild: Expression): Expression = Normalize(newChild)
