...
- `CreateJacksonParser.utf8String` — `createParser(new InputStreamReader(…))` — raw
  content is a `Reader`, so `case _` as well. This, not `.string`, is what
  `from_json` uses (`JsonExpressionEvalUtils.scala:145`); the branch taken and the
...
## 18. `writeRawValue` is reachable only through `VariantType`

> **Superseded design (decision 68).** This finding was recorded as an alternative to decision 62's marking, which stored decimals in their source lexical form. Lexical preservation is no longer attempted: a decimal is stored as the text of a double and written by casting that text back to a double, so the writer emits a bare number without a raw-value channel. The measurement stands; nothing depends on it.

Read from the Spark 4.0.1 sources in the local Maven repository (project targets
4.0.2); no script run for findings 18-21.

`JacksonGenerator` quotes every string it writes and offers no escape, per column
or otherwise — with one exception. `writeRawValue` appears exactly once in the
file, and a nested field reaches it (`JacksonGenerator.scala:208-209, 319-321`):
...
`makeWriter` builds array element writers too (`:67`), so `array<variant>` behaves
the same way. And a variant decimal renders with its **own** scale rather than a
column-wide one (`Variant.java:328-330`):
...
So `parse_json(<stored decimal text>)`, placed in the struct handed to `to_json`,
emits a bare unquoted number that preserves trailing zeros — `1.50` in, `1.50`
out — with no mark and no `regexp_replace` over the finished document. The
variant is transient: nothing is stored as `VariantType`, so the objections in
"Parquet variant shredding is not usable in Spark 4.0.2" do not apply.

**This is an alternative to decision 62, not a free tidy-up of it.** The fidelity
boundary is `VariantBuilder.tryParseDecimal` (`VariantBuilder.java:563-575`),
which admits only `-`, `.` and `0-9`, and only to precision and scale 38.
Anything else falls through to `appendDouble`.

| Stored text | Document today (mark + regex) | Document via `parse_json` |
|---|---|---|
| `1.50` | `1.50` | `1.50` |
| 40-digit plain | exact | exact to 38 digits, else double |
| `1.0e2` | `1.0e2` | `100.0` — form lost, value intact |
| `1.23456789012345678901e5` | exact | double, low digits differ |
| `1e400` | `1e400` | `"Infinity"` — quoted, so no longer a number |

The mark trick is exact on every row because it never parses. The variant route
trades that for removing the ugliness decision 62 records. The last row is only
reachable from input FHIR already disallows (finding 22).

A corollary: **there is no number-format control anywhere in the JSON writer.**
The generator is built bare — `new JsonFactory().createGenerator(writer)`
(`:78-79`) — with no Jackson `Feature` configured and nothing threaded through
from `JSONOptions`, whose full key list (`JSONOptions.scala:266-291`) carries
formatting options only for dates, times, ASCII escaping, pretty-printing and
line separators. `FloatType` and `DoubleType` go through `gen.writeNumber(…)`,
i.e. Java's `Double.toString`, which switches to exponent form below 10^-3 and at
or above 10^7 and cannot be steered.

## 19. `encoding` silently disables exact string parsing on the file path

> **No longer applies to Pathling (decision 68).** Exact string parsing mattered only while the ingest read primitives as text to preserve a decimal's lexical form. `primitivesAsString` is gone, so this trap can no longer be sprung here. It remains a correct observation about Spark.

The byte-exact branches in "Decimal lexical form by ingest path" depend on the
parser being created over a byte array or a `PositionedReadable`. Both file-read
paths pick their creator on whether `encoding` was set — `JsonDataSource.scala:142-143`
line-delimited, `:229-230` multiline:
...
and both encoding-aware creators wrap the input in a `Reader`
(`CreateJacksonParser.scala:60-78`): `text(enc, …)` through `getStreamDecoder`,
`inputStream(enc, …)` through `new InputStreamReader`. A `Reader` is neither
content-reference type, so the read falls to the re-serialising `case _`.

`JSONOptions.encoding` is `None` unless set — `parameters.get(ENCODING).orElse(parameters.get(CHARSET))`,
`JSONOptions.scala:162-163` — with no UTF-8 default, which is why the measured
JSONL row is byte-exact. But `getStreamDecoder` has no special case for `"UTF-8"`
either.

**So `.option("encoding", "UTF-8")` turns lexical preservation off.** It is
semantically a no-op — UTF-8 is what the byte path assumes anyway — and reads
like documentation rather than a behaviour change. `ResourceTransformer.source()`
does not set it today, and nothing would catch its addition: every ASCII decimal
without a trailing zero round trips identically either way. Worth a note beside
`DecimalTransform.lexicalReadOptions()`, whose javadoc already explains why the
option set lives where it does.

## 20. At JSON egress the writer drops one empty shape out of four

> **Superseded in part by decision 71.** The mechanism below is unchanged and is why the pruning was
> needed. `EmptyPruning` itself is gone: it grew the egress expression exponentially with nesting
> depth, and FR-019 is deferred with it, so three of the four shapes in the table are now written as
> they stand and the input is assumed not to carry them (T134h). The table's first two rows still
> describe what the writer does for free, and `ResourceTransformer` still applies `when(…isNotNull, …)`
> in both directions so that an absent structure stays absent. The T078c caveat below is now the
> serde's to answer rather than the filter's, because there is no filter.

The mechanism behind the observed `{"s":{}}` and `{"a":[null]}` rows in the JSON
egress table above. `ignoreNullFields` is consulted in exactly one place,
`writeFields` (`JacksonGenerator.scala:235-242`):

```scala
if (!row.isNullAt(i)) { gen.writeFieldName(field.name); fieldWriters(i).apply(row, i) }
else if (!options.ignoreNullFields || …) { gen.writeFieldName(field.name); gen.writeNull() }
```

`writeArrayData` (`:253-264`) does not consult it at all, and could not: JSON
arrays are positional, so there is no "omit" to express.

| Shape | Output | Automatic |
|---|---|---|
| null field | key omitted | yes — `ignoreNullFields` |
| null struct | key omitted | yes — same branch |
| non-null struct, every field null | `{}` | no |
| array containing null elements | `[…, null, …]` | no — no option reaches it |
| empty array | `[]` | no |

`functions.struct(…)` never yields a null, so without intervention *every*
optional nested structure in a definition-derived target emits `{}`.
`EmptyPruning` exists to convert the last three shapes into the first:
`when(populated, struct(fields))` for a structure, `filter(elements, isNotNull)`
plus a null at size zero for an array. `ResourceTransformer.structure` applies
the same `when` on ingest so that absence stays null in storage.

**Caveat for T078c.** FHIR JSON uses `null` inside an array legitimately: where a
repeating primitive carries ids or extensions, the value array and the parallel
`_<element>` array are padded with nulls to stay index-aligned.
`EmptyPruning.array`'s unconditional null filter is therefore correct only while
the metadata groups are excluded from the document — the exclusion
`ResourceSerialiser`'s javadoc records. When T078c starts writing them, the
filter has to become conditional on the paired array or the alignment shifts
silently. No test lacking a repeating primitive with a mid-array extension would
catch it.

## 21. No Spark or Parquet type distinguishes a lexical decimal from a string

> **Still live, and now load-bearing for a second reason (decision 68).** Egress must consult the definitions to know that a text column is a decimal and must be cast to a double for the writer — the same premise, reached by a different route now that marking is gone.

Evidence for `ResourceSerialiser`'s premise that "the schema cannot say what a
column means", and therefore that egress must consult the definitions.

| Candidate | Why it does not carry the distinction |
|---|---|
| `StringType` → Parquet `BYTE_ARRAY (STRING)` | identical to every FHIR `string`, `code` and `uri` |
| Parquet `DECIMAL` logical type | fixed point with one column-wide scale; over `BYTE_ARRAY` the bytes are two's-complement, not digits |
| `FIXED_LEN_BYTE_ARRAY`, unannotated | Spark cannot write one — only `DecimalType` maps to FLBA, always `DECIMAL`-annotated (`ParquetSchemaConverter.scala:684-690`) — and reads it back as `BinaryType`, dropping the length (`:330-336`). Fixed width also caps the digit count |
| `CHAR(n)` / `VARCHAR(n)` | not storage types: rewritten to `StringType` plus `__CHAR_VARCHAR_TYPE_STRING` field metadata (`CharVarcharUtils.scala:34,42-46`), with no case in the Parquet converter at all. `CHAR` pads with spaces on write and read; both raise past `n` |
| `StructField` metadata | does survive a Spark round trip — the whole `StructType` is serialised into the Parquet footer under `org.apache.spark.sql.parquet.row.metadata` and preferred over the Parquet schema on read (`ParquetFileFormat.scala:507-515`) — but it is a Spark-private side channel that no other reader consults |
| `VariantType` as storage | every variant column has the same field type; the tag is per value, not per field |

Parquet has no CHAR, VARCHAR or "text that is a number" logical type, and Spark's
type mapping is closed, so the only available carriers are a naming convention or
the definitions. The layout uses both: `__` and `_` prefixes for its own fields,
the definitions for everything else.

## 22. FHIR R4 permits exponent notation for `decimal`, and excludes `INF` / `NaN`

> **Still live, with a changed consequence (decision 68).** The exponent clause is what makes the stored text conformant, since a double renders small and large magnitudes as `1.0E-7` and `1.0E20`. The `INF`/`NaN` clause no longer indicts `PrimitiveValues`, whose lexical regexes are deleted; it is now an argument for the follow-up in T134g.

From the R4 specification (`hl7.org/fhir/R4/datatypes.html`), not from source.

The regex is `-?(0|[1-9][0-9]*)(\.[0-9]+)?([eE][+-]?[0-9]+)?`, which
`PrimitiveValues.LEXICAL_FORMS` carries verbatim. The XML representation is a
"union of xs:decimal and xs:double" — the union exists precisely because
`xs:decimal` has no exponent form. The spec also states that "the precision of
the decimal value has significance", `0.010` differing from `0.01`.

The regex is necessary but not sufficient:

> For decimal values, the XML special values `INF`, `-INF` and `NaN` are not
> allowed, and JSON is restricted to the precision limits documented in XML
> schema for `xs:double` and `xs:decimal`.

So `1e400` matches the regex and is **not** conformant: it exceeds the
`xs:double` range, and `INF` is the only value it could take.

`PrimitiveValues` checks the lexical form and nothing else, so it accepts such a
value and stores it. That is harmless under the current egress, which moves the
characters without parsing them, and a defect under any egress that parses
(finding 18, last row). It is a `StrictnessCheck` gap rather than a decimal-storage
one: the layout accepts what FHIR rejects.

## 23. A decimal through a double: what is stored and what is written

Measured on PySpark 4.0.1, script `scripts/double_serde.py`. This is the ingest
and egress path decision 68 adopts: read a number with standard JSON inference,
cast it to text for storage, and cast that text back to a double at egress so
the JSON writer emits a bare number.

The stored text is Java's `Double.toString`, and the writer renders the same
form:

| Source | Inferred | Stored | Written |
| --- | --- | --- | --- |
| `1.50` | `double` | `1.5` | `1.5` |
| `100`, in a file also holding a fraction | `double` | `100.0` | `100.0` |
| `0.0000001` | `double` | `1.0E-7` | `1.0E-7` |
| `1e20` | `double` | `1.0E20` | `1.0E20` |
| `1.5e2` | `double` | `150.0` | `150.0` |
| forty significant digits | `double` | `1.2345678901234568E39` | `1.2345678901234568E39` |
| `-2.5` | `double` | `-2.5` | `-2.5` |
| `100`, in a file whose decimals are all integral | `bigint` | `100` | `100.0` |

Four things follow.

**Every stored form is conformant.** FHIR R4's decimal regex permits an exponent
with an optional sign, so `1.0E-7`, `1.0E20` and `1.2345678901234568E39` all
match, as do `1.5` and `100.0`. The stored text is what the engine and any
external reader parse, so this is the property that had to hold.

**The stored text depends on the file's other values.** A file whose decimal
values are all integral infers `bigint`, and `100` stores as `100`; the same
value in a file containing any fraction infers `double` and stores as `100.0`.
Both parse to the same decimal, so nothing downstream is affected, but the
stored bytes for one resource are not a function of that resource alone.

**Egress never writes a bare integer.** Because the cast is to a double, a
source `100` returns as `100.0` whichever way it was stored. Conformant, and
numerically equal.

**`try_cast` is what makes an out-of-range integer an absence rather than an
error.** `try_cast(3000000000L AS INT)` is null and `try_cast(7L AS INT)` is 7.
Inference never produces `IntegerType`, so an `integer` element always arrives
as `bigint` and this is the only thing standing between a large value and either
an ANSI failure or a silently wrong one.
