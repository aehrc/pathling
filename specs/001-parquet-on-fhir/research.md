...
> **Overtaken by decision 68.** The question this entry answers — how to
> preserve a decimal's lexical form, and on which paths it can be done — is no
> longer asked. Lexical form is not preserved on any path: ingest reads a number
> with standard JSON inference and stores its text, so the file path and the
> `Dataset<String>` path agree, and the guarantee is numeric equivalence. The
> measurements below stand and are why the question could be closed rather than
> solved; the quoting pre-pass (T134) is withdrawn with it. The variant
> alternative is still benchmarked (T135), on performance alone.
