...
| 16 US6 Primitive ids and extensions                                  | T051, T061, T061a, T078c, T120–T124, T124a–T124j                                                 | 19    |
...
of the derivation: the transform's traversal _is_ a walk of it, so there is one
...
- [x] T018 _Withdrawn by decision 67._ The reported type stays the R4 enumeration; no module-local representation is introduced.
- [x] T019 _Withdrawn by decision 67._ The engine's call sites are unchanged beyond T011's imports. The engine continues to read cardinality from the Spark schema in this phase.
...
- [ ] T061a [US6] **New, from the M1 review (#2767).** Give a metadata group's extensions the shape of the Extension type itself, in `fhir-schema/src/main/java/au/csiro/pathling/schema/DefinitionCanonicalStructure.java`. `metadataGroup` takes that shape from the `extension` element of the node holding the primitive, and a resource that is not a domain resource — `Binary`, `Bundle`, `Parameters` — has none at its root. `_id`, `_language`, `Bundle._timestamp` and `Binary._contentType` therefore have no canonical structure beneath their `extension`, so a merge orders its fields by name rather than by definition (FR-057), and this task's transform would have no shape to write them in. Latent in M1, where nothing writes a group's content or merges a schema that carries one. Test it on all three resources in `DefinitionCanonicalStructureTest`.
...
### From the M1 review (#2767)

_Findings on the M1 pull request that M1 does not need, recorded here so that
none is rediscovered. Each is either latent until something in M5 or later
reaches it, or is tidying that does not change what M1 writes._

- [ ] T124a **New, from the M1 review.** Report, rather than null, a value the converter cannot store, in `io/src/main/java/au/csiro/pathling/io/transform/PrimitiveConverters.java`. An `integer`, `unsignedInt` or `positiveInt` beyond the range of an int arrives as a long column, which the converter accepts, and `try_cast` then stores null with no finding; a malformed `base64Binary` is nulled by `try_to_binary` the same way. Both are losses FR-017 requires to be detectable, and `StrictnessCheck` passes them because it examines types rather than values.
- [ ] T124b **New, from the M1 review.** Decide whether the JSON reader infers with `prefersDecimal`, in `io/src/main/java/au/csiro/pathling/io/json/FhirJsonReader.java`. Without it every decimal passes through a double on ingest, and the `DECIMAL` converter already accepts `DecimalType`, so up to 38 digits would survive ingest exactly; egress still casts to double (decision 68), so this bears on T134h rather than replacing it. Two things are wrong whichever way it goes: `PrimitiveTypes`' javadoc claims the stored text is numerically equal to the source, and `SemanticJson.difference` compares numbers through `doubleValue()`, so the round-trip harness cannot see a decimal lose digits. Make the comparison exact, and let the exception list say what is lost.
- [ ] T124c **New, from the M1 review.** Make egress ignore and report a stored field the canonical structure does not know, rather than throwing, in `ResourceTransformer.toJsonShape`. Ingest already ignores and reports such content. A merged schema carrying a field the merge appended, a file from another producer carrying `contained`, or a dataset in the previous layout (`_fid`, `_extension`) otherwise fails the whole write on its first unknown column. T061 is where egress first meets files it did not write itself.
- [ ] T124d **New, from the M1 review.** Pin FHIRPath's `type()` over a choice element in `fhirpath`, including a choice that can hold a reference such as `Extension.value`. `FhirChoiceDefinition.getAllChildTypes` changed in M1 to declared order with one reference variant, and it also feeds `TypeInfo.choiceTypeInfoMapper`, so the engine's behaviour moved with no engine test to show it. Run the test against the previous definition code as well, to confirm the two agree.
- [ ] T124e **New, from the M1 review.** Key the canonical structure memo on the `resource` flag as well as the type identity, or assert that one type is never reached both ways, in `DefinitionCanonicalStructure`. The flag decides whether `resourceType` is added and `contained` dropped, and a cached structure is returned whichever flag the second request carried. Theoretical today, because nothing inside a resource reaches the same resource definition again.
- [ ] T124f **New, from the M1 review.** Remove the duplicate cardinality accessor on `DefaultCompositeDefinition` and `DefaultPrimitiveDefinition`, where `getMaxCardinality()` sits beside the Lombok `getCardinality()` for the same field. Rename the field and migrate the few callers, `YamlSupport` among them.
- [ ] T124g **New, from the M1 review.** Stop the invocation-level setup that prepares the view execution benchmark from also running before every view planning invocation, in `benchmark/src/main/java/au/csiro/pathling/benchmark/PathlingBenchmarkState.java`. It plans the same query immediately before the measured plan, so view planning is measured warm and under-reported. Give the execution preparation a `@State` of its own. This bears on T136, which orders this milestone by what the benchmark measures.
- [ ] T124h **New, from the M1 review.** Remove the `ndjson` source branch and `initialiseNdjsonSource` from the same class, or restore `ndjson` to the source `@Param`. The parameter was narrowed to `delta`, leaving the branch unreachable and untested.
- [x] T124i **New, from the M1 review.** Add `fhir-schema` and `io` to the documented module structure: the build order in `.claude/CLAUDE.md`, and the module list and dependency graph in `CONTRIBUTING.md`, including `fhirpath`'s new dependency on `fhir-schema`.
- [ ] T124j **New, from the M1 review.** Report a number or boolean that JSON inference stores as text, in `io/src/main/java/au/csiro/pathling/io/transform/StrictnessCheck.java`. Spark types a column once per file, so where one document carries `"gender":123` or `"birthDate":2000` and another a string for the same element, the column is inferred as a string, every string-like converter accepts it, and the number is stored and written back as `"123"` with no finding. Alone in its file the same value is a `bigint` column and is ignored and reported, as FR-018 requires; beside a string it is neither. The per-column check cannot see it, because the source's JSON type is gone once the column is inferred. Decide how to detect it — from the text, or by inferring each document apart — and pin both the reported case and the conformant string beside it in `StrictnessTest`. T134g's per-value validation is related but does not cover it: the value is kept, not nulled.
...
| FR-018 _Withdrawn for this phase by decision 68_; ignoring is the only mode and is reported                          | T057, T067, T124j (M5), T134g (the follow-up)                                                                                                                                                                                                                               |
| FR-019 _Deferred by decision 71_; a null field is left out, an empty structure or array is not                       | T080a, T080b, T080c, T134h (the follow-up)                                                                                                                                                                                                                                  |
| FR-020 _Withdrawn by decision 68_; no lexical form is preserved on any path                                          | —                                                                                                                                                                                                                                                                           |
