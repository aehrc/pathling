...
- [ ] T124i **New, from the M1 review.** Add `fhir-schema` and `io` to the documented module structure: the build order in `.claude/CLAUDE.md`, and the module list and dependency graph in `CONTRIBUTING.md`, including `fhirpath`'s new dependency on `fhir-schema`.
