...
| ----------------------------------------------------------------------------------------- | ------ | ----- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **M1 The layout** [#2761](https://github.com/aehrc/pathling/issues/2761)                  | 1–6    | 64    | FHIR JSON to the new layout and back through `io`, losslessly except primitive ids and extensions. No annotations. Schema derivation, the canonical structure and the one shared merge. Both risk gates resolved | None                                                                                                                                                                                                                                                           |
...
| 4 US1 The storage layout                                             | T050, T052, T056, T057, T057a, T057b, T057c, T059, T059a, T060, T062, T067, T067a                | 13    |
| 5 US2 Lossless round trip                                            | T071–T078, T078a, T078b, T078d, T079, T080                                                       | 13    |
...
- [x] T057c [P] [US1] Test that a primitive value contradicting its declared type fails the transform in both settings of the strictness switch and both settings of `spark.sql.ansi.enabled`, naming the element and its type, in `io/src/test/java/au/csiro/pathling/io/transform/PrimitiveValueTest.java`. The cases are the ones a plain cast gets wrong: `yes` for a boolean, which Spark casts to true even with ANSI on; `1.5` for an integer, which it truncates to 1 with ANSI off; an integer out of range, a negative `unsignedInt`, a zero among a repeating `positiveInt`, and text in a decimal, which no cast sees at all and whose marks egress would leak. A positive control stores values at the edge of each type's range under both settings. Added by M1's first review; decision 57 records what it found.
...
- [x] T059a [US1] Implement the check of a primitive value against the lexical form the R4 definitions declare for its type, with a cast that does not depend on ANSI mode, in `io/src/main/java/au/csiro/pathling/io/transform/PrimitiveValues.java`. A failure raises an error naming the element's path and declared type, and not the value, which is patient data. The path is threaded through `ResourceTransformer` for that alone.
...
- [ ] T078c [US6] Re-assert FR-017 unconditionally: remove the primitive-metadata exclusion from the round-trip harness and from T076, T077 and T078a, and delete the carve-out from the spec. This includes the cascade the exclusion carries — the removal of containers the exclusion itself empties — which exists only because the metadata is withheld and is wrong once it is written. The `contained` exclusion beside it is **not** part of this: FR-006 is permanent. This is what closes decision 49.
