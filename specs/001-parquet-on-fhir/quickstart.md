...
object key order ignored, array order significant, numbers compared
**numerically** (decision 68) — subject to the exceptions FR-016 lists.
...
small magnitude, and one with forty significant digits. All five must be
numerically equal; none will be textually identical, because a decimal is
stored as the text of a double. Also a resource with an element absent, to
confirm it is absent from the output rather than null, and a resource where a
structure's fields are all empty, to confirm it does not serialise as an empty
object.

**Also expect**: supplying the same resources as a dataset of strings rather
than as files now gives the same result as supplying them as files, because
both route a number through a double. The per-path limitation this scenario
used to assert is withdrawn with FR-020.
...
**Expect at M1 through M4**: the same, except that neither the annotation fields
nor the `_field` groups appear at all. Decision 68 deleted the derived schema
that carried them unpopulated, and on a schema fitted to the data an element
nothing writes has no column, which is what FR-017's carve-out says and
`StrictnessTest.dropsPrimitiveMetadataFromTheFittedSchema` asserts. The layout
ships annotation-free at the flip (decision 50) and the metadata group is
written in M5 (decision 49), so this scenario is run in two parts rather than
deferred whole.
...
any dataset, then apply it to this fitted dataset and to a second dataset holding
the same resources under a different schema — two fitted schemas of differing
width at the flip, and a dense one once M6 lands (decision 69). The results must
be equal. This is what makes the
...
chosen approach, and direct parsing into a variant. The comparison stands on
performance alone: decision 68 removed the lexical-decimal limitation that was
the other reason to consider changing mechanism.
