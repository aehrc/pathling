 ...
 * built as a local relation of its members, and a {@code SQLView} node's SQL is rewritten against
 * the temp views of its already-materialised children, validated, and run. The top-level SQL is
 * then rewritten against its own direct dependencies' temp views and run. Every node's SQL is
 * validated statically before execution and against its analysed plan during execution.
 *
 * @author John Grimes
 */
@Slf4j
...
public class SqlQueryExecutor {
   ...
   * is read from its configured path, a value set leaf is built from its membership, and a {@code
   * SQLView} node's analysed plan is validated against its own children's temp views before
   * registration, so it cannot reach an unauthorised data source. No leaf kind needs that check: a
   * leaf's relation is exposed only through the trusted alias registered here, which is what the
   * parent's own analysed-plan check accepts.
   */
