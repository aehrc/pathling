...
public class ExportManifest {
  ...
  @Nonnull private final List<ExportManifestOutput> outputs;

  /**
   * Creates a new ExportManifest.
   *
   * @param serverBaseUrl the FHIR server base URL (used for constructing the result download URLs)
   ...
   * @param format the effective output format code (e.g. {@code ndjson})
   ...
   * @param outputs the export outputs, one per exported unit, in order
   */
  ...
  public ExportManifest(
      ...
      @Nonnull final List<ExportManifestOutput> outputs) {
  ...
  /**
   * Builds the completion manifest as a FHIR {@code Parameters} resource.
   *
   * @return the manifest Parameters
   */
  @Nonnull
  public Parameters toParameters() {
    ...
    // One output per exported unit, with a name and one or more location parts.
    for (final ExportManifestOutput output : outputs) {
  ...
  /**
   * Converts a local file URL to a remote {@code $result} download URL.
   *
   * @param baseUrl the normalised base server URL (ending in a slash)
